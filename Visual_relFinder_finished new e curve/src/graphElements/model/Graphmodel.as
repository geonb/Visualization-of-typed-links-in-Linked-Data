package graphElements.model 
{
	...
	public class Graphmodel extends EventDispatcher
	{
		...
		private var givenNodesArray:Array = new Array();
		...
		private function addRelationToGraph(subjectNode:MyNode, predicateNode:RelationNode, objectNode:MyNode, layout:Object):void {
			...
			_completeGraph.link(predicateNode, subjectNode, object2);	
			
			var object1:Object = new Object();
			object1.startId = objectNode.id;	//defines the direction of the link!
			if (layout != null) object1.settings = layout.settings;
			_completeGraph.link(objectNode, predicateNode, object1);
			//applyZoomFactor();
		...
		public function drawPath(p:Path, immediatly:Boolean = false):void {
				...
				for (it = 0; it < p.relations.length; it++) {
					drawRelation(p.relations[it], p.layout);
				}
			}
		}
		...
		private function drawRelation(r:Relation, layout:Object):void {
			arri.push(r);
			rel = func_check(arri, r);
			var angle:Number = 360 / arri.length;
			centerX = app.sGraph.width / 2;
			centerY = app.sGraph.height / 2;
			var a1:Number = centerX;
			var b1:Number = centerY;
			var predicateNode:RelationNode = getRelationNode(rel.id, rel); //important: _r.id and not _r.predicate.id!!
			predicateNode.setPosition(centerX - 90, -4*26 + centerY + (itb % 12) * 26);
			...
			objectNode.setPosition(270*Math.cos((7*(ita-itx) -  180) * (Math.PI / 180)) + centerX,270*Math.sin((7*(ita-itx) -180) * (Math.PI / 180)) + centerY);
			if (!_completeGraph.hasNode(objectNode.id)) {
				showNode(objectNode);
			}
			var subjectNode:MyNode = getInstanceNode(rel.subject.id, rel.subject);
			subjectNode.setPosition(270*Math.cos((7*(ita-itx) -  180) * (Math.PI / 180)) + centerX,270*Math.sin((7*(ita-itx) -180) * (Math.PI / 180)) + centerY);
