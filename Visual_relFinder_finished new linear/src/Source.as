...
import graphElements.GivenNodeView;
import graphElements.FoundNodeView;
import graphElements.RelationNodeView;
...
public function zoomSliderUp():void {
	...
	if (sliderDamper > 1 && zoomSlider.value < Graphmodel.ZOOM_MAXIMUM) {
		sliderDamper = 0;
		zoomSlider.value += 1;
		(sGraph as Canvas).scaleX = 1.0 / zoomSlider.value;
		(sGraph as Canvas).scaleY = 1.0 / zoomSlider.value;
...
public function zoomSliderDown():void {
	...
	if (sliderDamper > 1 && zoomSlider.value > Graphmodel.ZOOM_MINIMUM) {
		sliderDamper = 0;
		zoomSlider.value -= 1;
		(sGraph as Canvas).scaleX = 1.0 / zoomSlider.value * 2;
		(sGraph as Canvas).scaleY = 1.0 / zoomSlider.value * 2;
		graphModel.zoomFactor = zoomSlider.value;
